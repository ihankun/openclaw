...
const STATUS_TABS: StatusTabDef[] = [
  { id: "all", labelKey: "skillsView.tabs.all" },
  { id: "ready", labelKey: "skillsView.tabs.ready" },
  { id: "needs-setup", labelKey: "skillsView.tabs.needsSetup" },
  { id: "disabled", labelKey: "skillsView.tabs.disabled" },
];
...
function verdictLabel(verdict: ClawHubSkillSecurityVerdict | null | undefined): string {
  if (!verdict) {
    return t("skillsView.verdict.unavailable");
  }
  const status = verdict.securityStatus?.trim() || null;
  if (verdict.ok && verdict.decision === "pass") {
    return status === "clean" || !status ? t("skillsView.verdict.clean") : status;
  }
  if (status === "pending" || status === "not-run") {
    return t("skillsView.verdict.pending");
  }
  if (status === "malicious") {
    return t("skillsView.verdict.blocked");
  }
  if (status === "suspicious") {
    return t("skillsView.verdict.review");
  }
  return t("skillsView.verdict.unavailable");
}
...
function agentOptionLabel(agent: SkillsAgentOption, defaultId: string | undefined): string {
  ...
  return agent.id === defaultId ? t("skillsView.defaultAgent", { name: baseName }) : baseName;
}

function skillGroupLabel(groupId: string): string {
  switch (groupId) {
    case "workspace":
      return t("skillsView.groups.workspace");
    case "built-in":
      return t("skillsView.groups.builtIn");
    case "installed":
      return t("skillsView.groups.installed");
    case "extra":
      return t("skillsView.groups.extra");
    case "other":
      return t("skillsView.groups.other");
    default:
      return groupId;
  }
}
...
function skillReasonLabel(reason: string): string {
  switch (reason) {
    case "disabled":
      return t("skillsView.reasons.disabled");
    case "blocked by allowlist":
      return t("skillsView.reasons.blockedByAllowlist");
    case "blocked by agent filter":
      return t("skillsView.reasons.blockedByAgentFilter");
    default:
      return reason;
  }
}
      ...
      >
        ...
        <label class="field" style="flex: 1; min-width: 180px;">
          <input
            ...
            placeholder=${t("skillsView.filterPlaceholder")}
            autocomplete="off"
            name="skills-filter"
          />
        </label>
        <div class="muted">${t("skillsView.shown", { count: String(filtered.length) })}</div>
      </div>

      <div style="margin-top: 16px; border-top: 1px solid var(--border); padding-top: 16px;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
          <div style="font-weight: 600;">ClawHub</div>
          <div class="muted" style="font-size: 13px;">${t("skillsView.clawhub.description")}</div>
        </div>
        <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
          <label class="field" style="flex: 1; min-width: 180px;">
            <input
              ...
              placeholder=${t("skillsView.clawhub.searchPlaceholder")}
              autocomplete="off"
              name="clawhub-search"
            />
          </label>
          ${props.clawhubSearchLoading
            ? html`<span class="muted">${t("skillsView.clawhub.searching")}</span>`
            : nothing}
        </div>
              ...
              ${props.clawhubInstallMessage.acknowledgeSlug
                ? html`<button
                  ...
                  >
                    ...
                    t("skillsView.clawhub.acknowledgeRisk")}
                  </button>`
                : nothing}
            </div>`
          : nothing}
        ...
        ? html`
            <div class="muted" style="margin-top: 16px">
              ${!props.connected && !props.report
                ? t("skillsView.empty.disconnected")
                : t("skillsView.empty.noSkills")}
            </div>
          `
        : html`
                ...
                return html`
                  <details class="agent-skills-group" open>
                    <summary class="agent-skills-header">
                      <span>${skillGroupLabel(group.id)}</span>
...
function renderClawHubResults(props: SkillsProps) {
  ...
  if (results.length === 0) {
    return html`<div class="muted" style="margin-top: 8px;">
      ${t("skillsView.clawhub.noResults")}
    </div>`;
  }
  return html`
            ...
            <div class="list-meta" style="display: flex; align-items: center; gap: 8px;">
              ...
              >
                ${props.clawhubInstallSlug === r.slug
                  ? t("skillsView.clawhub.installing")
                  : t("skillsView.clawhub.install")}
              </button>
            </div>
          </div>
        `,
                ...
                ? html`
                    ...
                    ${detail.metadata?.os
                      ? html`<div class="muted" style="font-size: 12px;">
                          ${t("skillsView.clawhub.platforms", {
                    ...
                    >
                      ${props.clawhubInstallSlug === props.clawhubDetailSlug
                        ? t("skillsView.clawhub.installing")
                        : t("skillsView.clawhub.installNamed", {
                            name: detail.skill.displayName,
                          })}
                    </button>
                  `
                : html`<div class="muted">${t("skillsView.clawhub.notFound")}</div>`}
        </div>
      </div>
    </dialog>
  `;
      ...
      >
        ${skill.clawhub?.status === "linked"
          ...
          : skill.clawhub?.status === "invalid"
            ? html`<span class="chip chip-warn">${t("skillsView.clawhub.linkInvalid")}</span>`
            : nothing}
    ...
    >
      <div class="md-preview-dialog__panel">
        ...
        <div class="md-preview-dialog__body" style="display: grid; gap: 16px;">
            ...
            ? html`
                ...
                >
                  <div style="font-weight: 600; margin-bottom: 4px;">
                    ${t("skillsView.detail.missingRequirements")}
                  </div>
          ...
          ${reasons.length > 0
            ? html`
                <div class="muted" style="font-size: 13px;">
                  ${t("skillsView.detail.reason", {
                    reasons: reasons.map(skillReasonLabel).join(", "),
                  })}
                </div>
              `
            : nothing}
          ...
          <div style="display: flex; align-items: center; gap: 12px;">
            ...
            ${canInstall
              ? html`<button
                ...
                >
                  ${busy ? t("skillsView.clawhub.installing") : skill.install[0].label}
                </button>`
              : nothing}
          </div>
          ...
          >
            <div>
              <span style="font-weight: 600;">${t("skillsView.detail.source")}</span>
              ${skill.source}
            </div>
...
function renderInstalledClawHubOverview(
  ...
  if (link.status === "invalid") {
    return html`<div class="callout danger">
      <div style="font-weight: 600; margin-bottom: 4px;">
        ${t("skillsView.clawhub.linkInvalid")}
      </div>
      <div>${link.reason}</div>
    </div>`;
  }
    ...
    >
      <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
        ...
        ${props.clawhubVerdictsLoading
          ? html`<span class="muted">${t("skillsView.clawhub.refreshing")}</span>`
          : nothing}
      </div>
      ...
      ${auditHref
        ? html`<div style="font-size: 13px;">
            <a href="${auditHref}" target="_blank" rel="noopener noreferrer"
              >${t("skillsView.clawhub.fullSecurityReport")}</a
            >
          </div>`
        : nothing}
    </div>
  ...
  if (content === undefined) {
    ...
    return html`<div class="muted" style="font-size: 13px;">
      ${props.skillCardLoadingKey === skill.skillKey
        ? t("skillsView.detail.loadingCard")
        : t("skillsView.detail.cardNotLoaded")}
    </div>`;
  }
  return html`
